...
class SfCartesianChart extends StatefulWidget {
  ...
  final Function(String, String, String)? onAreaCallback;
...
///
class SfCartesianChartState extends State<SfCartesianChart> with TickerProviderStateMixin {
  late CartesianStateProperties _stateProperties;

  /// To initialize default values
  void _initializeDefaultValues() {
    _stateProperties = CartesianStateProperties(renderingDetails: RenderingDetails(), chartState: this);
    ...
    _stateProperties.technicalIndicatorRenderer = <TechnicalIndicatorsRenderer>[];
    _stateProperties.zoomPanBehaviorRenderer = ZoomPanBehaviorRenderer(_stateProperties);
    _stateProperties.trackballBehaviorRenderer = TrackballBehaviorRenderer(_stateProperties);
    _stateProperties.crosshairBehaviorRenderer = CrosshairBehaviorRenderer(_stateProperties);
    ...
    _stateProperties.trackballMarkerSettingsRenderer = TrackballMarkerSettingsRenderer(widget.trackballBehavior.markerSettings);

    _stateProperties.isArea = widget.isArea;
    _stateProperties.onAreaCallback = widget.onAreaCallback;

    final TargetPlatform platform = defaultTargetPlatform;
    _stateProperties.enableMouseHover =
        kIsWeb || platform == TargetPlatform.windows || platform == TargetPlatform.macOS || platform == TargetPlatform.linux;
  }
  ...
  void didChangeDependencies() {
    ...
    _stateProperties.renderingDetails.isRtl = Directionality.of(context) == TextDirection.rtl;
  ...
  void didUpdateWidget(SfCartesianChart oldWidget) {
    ...
    final List<CartesianSeriesRenderer> oldWidgetOldSeriesRenderers = <CartesianSeriesRenderer>[]
      //ignore: prefer_spread_collections
    ...
    _createAndUpdateSeriesRenderer(oldWidget, oldWidgetSeriesRenderers, oldWidgetOldSeriesRenderers);
    needsRepaintChart(_stateProperties, _stateProperties.chartAxis.axisRenderersCollection, oldWidgetSeriesRenderers);
    _stateProperties.renderingDetails.isLegendToggled = false;
    // ignore: unnecessary_null_comparison
    if (_stateProperties.renderingDetails.legendWidgetContext != null && _stateProperties.renderingDetails.legendWidgetContext.isNotEmpty) {
      _stateProperties.renderingDetails.legendWidgetContext.clear();
    }
    final SeriesRendererDetails? seriesRendererDetails =
        _stateProperties.seriesRenderers.isNotEmpty ? SeriesHelper.getSeriesRendererDetails(_stateProperties.seriesRenderers[0]) : null;
    if (seriesRendererDetails?.selectionBehaviorRenderer != null) {
      final SelectionDetails? selectionDetails = SelectionHelper.getRenderingDetails(seriesRendererDetails!.selectionBehaviorRenderer!);
      if (_stateProperties.seriesRenderers.isNotEmpty && selectionDetails?.selectionRenderer != null) {
  ...
  @override
  Widget build(BuildContext context) {
    _stateProperties.renderingDetails.oldDeviceOrientation = _stateProperties.renderingDetails.oldDeviceOrientation == null
    ...
    _stateProperties.renderingDetails.initialRender = _stateProperties.renderingDetails.initialRender == null;
    ...
    return RepaintBoundary(
      ...
      decoration: BoxDecoration(
          color: widget.backgroundColor ?? _stateProperties.renderingDetails.chartTheme.backgroundColor,
  ...
  Future<dart_ui.Image> toImage({double pixelRatio = 1.0}) async {
    final RenderRepaintBoundary boundary = context.findRenderObject() as RenderRepaintBoundary; //get the render object from context

    final dart_ui.Image image = await boundary.toImage(pixelRatio: pixelRatio); // Convert
  ...
  void _getOldSeriesKeys(List<CartesianSeriesRenderer> oldSeriesRenderers) {
    ...
    for (int i = 0; i < oldSeriesRenderers.length; i++) {
      _stateProperties.oldSeriesKeys.add(SeriesHelper.getSeriesRendererDetails(oldSeriesRenderers[i]).series.key);
    }
  }
    ...
    if (widget.series != null && widget.series.isNotEmpty) {
      ...
      for (int i = 0; i < seriesLength; i++) {
        ...
        if (oldWidget != null) {
          ...
          if (oldWidgetSeriesRenderers!.isNotEmpty) {
            if (oldWidgetSeriesRenderers.length > i) {
              final SeriesRendererDetails seriesRendererDetails = SeriesHelper.getSeriesRendererDetails(oldWidgetSeriesRenderers[i]);

              /// Disposing the old series segment objects.
              if (oldWidget.series.length == widget.series.length &&
                  oldWidget.series[i].dataSource == widget.series[i].dataSource &&
                  seriesRendererDetails.dataPoints.length == widget.series[i].dataSource!.length) {
                disposeOldSegments(widget, SeriesHelper.getSeriesRendererDetails(oldWidgetSeriesRenderers[i]));
              }
            }

            oldSeriesIndex =
                i < oldWidgetSeriesRenderers.length && isSameSeries(SeriesHelper.getSeriesRendererDetails(oldWidgetSeriesRenderers[i]).series, series)
                    ? i
        ...
        if (index != null &&
          ...
          SeriesHelper.setSeriesRendererDetails(seriesRenderer, SeriesRendererDetails(seriesRenderer));
          ...
          if (seriesRenderer is XyDataSeriesRenderer) {
            seriesRendererDetails.animationController = AnimationController(vsync: this)..addListener(seriesRendererDetails.repaintSeriesElement);
            _stateProperties.controllerList[seriesRendererDetails.animationController] = seriesRendererDetails.repaintSeriesElement;
            seriesRendererDetails.animationController.addStatusListener(seriesRendererDetails.animationStatusListener);
          }
          seriesRendererDetails.controller ??= ChartSeriesController(seriesRenderer as XyDataSeriesRenderer);
        }
        final SeriesRendererDetails seriesRendererDetails = SeriesHelper.getSeriesRendererDetails(seriesRenderer);
        ...
        seriesRendererDetails.isSelectionEnable = series.selectionBehavior.enable == true;
        ...
        if (oldWidgetSeriesRenderers != null && oldSeriesIndex != null && oldWidgetSeriesRenderers.length > oldSeriesIndex) {
          final SeriesRendererDetails oldSeriesRendererDetails = SeriesHelper.getSeriesRendererDetails(oldWidgetSeriesRenderers[oldSeriesIndex]);
          seriesRendererDetails.oldSeries = oldSeriesRendererDetails.series;
          if (seriesRenderer is FastLineSeriesRenderer && oldWidgetSeriesRenderers[oldSeriesIndex] is FastLineSeriesRenderer) {
            final FastLineSeriesRenderer fastlineSeriesRenderer = oldWidgetSeriesRenderers[oldSeriesIndex] as FastLineSeriesRenderer;
            seriesRendererDetails.oldDataPoints = <CartesianChartPoint<dynamic>>[]
              //ignore: prefer_spread_collections
              ..addAll(SeriesHelper.getSeriesRendererDetails(fastlineSeriesRenderer).overallDataPoints);
          } else {
            seriesRendererDetails.oldDataPoints = <CartesianChartPoint<dynamic>>[]
              //ignore: prefer_spread_collections
              ..addAll(oldSeriesRendererDetails.dataPoints);
          }
          seriesRendererDetails.oldSelectedIndexes = oldSeriesRendererDetails.oldSelectedIndexes;
          seriesRendererDetails.repaintNotifier = oldSeriesRendererDetails.repaintNotifier;
  ...
  int? _getExistingSeriesIndex(CartesianSeries<dynamic, dynamic> currentSeries, List<CartesianSeriesRenderer> oldSeriesRenderers) {
    if (currentSeries.key != null) {
      for (int index = 0; index < oldSeriesRenderers.length; index++) {
        final CartesianSeries<dynamic, dynamic> series = SeriesHelper.getSeriesRendererDetails(oldSeriesRenderers[index]).series;
  ...
  void _refresh() {
    if (_stateProperties.renderingDetails.legendWidgetContext.isNotEmpty) {
      for (int i = 0; i < _stateProperties.renderingDetails.legendWidgetContext.length; i++) {
        final MeasureWidgetContext templateContext = _stateProperties.renderingDetails.legendWidgetContext[i];
        final RenderBox renderBox = templateContext.context!.findRenderObject() as RenderBox;
  ...
  Widget _renderTitle() {
    Widget titleWidget;
    // ignore: unnecessary_null_comparison
    if (_stateProperties.chart.title.text != null && _stateProperties.chart.title.text.isNotEmpty) {
      ...
      final TextStyle titleStyle = getTextStyle(
          ...
          fontColor: _stateProperties.chart.title.textStyle.color ?? _stateProperties.renderingDetails.chartTheme.titleTextColor);
      ...
      titleWidget = Container(
        alignment: (_stateProperties.chart.title.alignment == ChartAlignment.near)
            ? Alignment.topLeft
            : (_stateProperties.chart.title.alignment == ChartAlignment.far)
                ? Alignment.topRight
                : (_stateProperties.chart.title.alignment == ChartAlignment.center)
                    ? Alignment.topCenter
                    : Alignment.topCenter,
        child: Container(
          decoration: BoxDecoration(
              color: _stateProperties.chart.title.backgroundColor ?? _stateProperties.renderingDetails.chartTheme.titleBackgroundColor,
              border: Border.all(
                  color: _stateProperties.chart.title.borderWidth == 0
                      ? Colors.transparent
                      : _stateProperties.chart.title.borderColor, // ?? _chartTheme.titleTextColor,
  ...
  Widget _renderChartElements(BuildContext context) {
    ...
    return Expanded(
      child: LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
        ...
        _stateProperties.renderingDetails.prevLocale = _stateProperties.renderingDetails.prevLocale ?? currentLocale;
        _stateProperties.renderingDetails.prevSize = _stateProperties.renderingDetails.prevSize ?? constraints.biggest;
        _stateProperties.renderingDetails.didSizeChange = _stateProperties.renderingDetails.prevSize != constraints.biggest;
        _stateProperties.renderingDetails.didLocaleChange = _stateProperties.renderingDetails.prevLocale != currentLocale;
        ...
        final CartesianChartPoint<dynamic> crosshairPoint = _getCrosshairChartPoint(_stateProperties);
        ...
        if (legendTemplates.isNotEmpty && _stateProperties.renderingDetails.legendWidgetContext.isEmpty) {
          ...
          _stateProperties.renderingDetails.chartLegend.calculateLegendBounds(_stateProperties.renderingDetails.chartLegend.chartSize);
          _stateProperties.containerArea = ContainerArea(_stateProperties);
          element = getElements(_stateProperties, _stateProperties.containerArea, constraints);
        }
        return element!;
      }),
    );
  ...
  List<Widget> _bindCartesianLegendTemplateWidgets() {
    ...
    if (widget.legend.isVisible! && widget.legend.legendItemBuilder != null) {
      for (int i = 0; i < _stateProperties.seriesRenderers.length; i++) {
        final CartesianSeriesRenderer seriesRenderer = _stateProperties.seriesRenderers[i];
        final SeriesRendererDetails seriesRendererDetails = SeriesHelper.getSeriesRendererDetails(seriesRenderer);
        if (seriesRendererDetails.series.isVisibleInLegend == true) {
          legendWidget = widget.legend.legendItemBuilder!(seriesRendererDetails.seriesName!, seriesRendererDetails.series, null, i);
  ...
  void _initialize(BoxConstraints constraints) {
    ...
    final bool isMobilePlatform = defaultTargetPlatform == TargetPlatform.android || defaultTargetPlatform == TargetPlatform.iOS;
    ...
    final LegendPosition position = _stateProperties.renderingDetails.legendRenderer.legendPosition;
    ...
    _stateProperties.renderingDetails.chartLegend.chartSize = Size(width - widthPadding, height - heightPadding);
  }

  /// To find the visible series
  void _findVisibleSeries(BuildContext context) {
    final List<CartesianSeriesRenderer> seriesRenderers = _stateProperties.seriesRenderers;
    bool legendCheck = false;
    _stateProperties.chartSeries.visibleSeriesRenderers = <CartesianSeriesRenderer>[];
    final List<CartesianSeriesRenderer> visibleSeriesRenderers = _stateProperties.chartSeries.visibleSeriesRenderers;
    for (int i = 0; i < seriesRenderers.length; i++) {
      ...
      seriesRendererDetails.seriesName = seriesRendererDetails.series.name ?? '${SfLocalizations.of(context).series} $i';
      final CartesianSeries<dynamic, dynamic> cartesianSeries = seriesRendererDetails.series;
      ...
      if (cartesianSeries.trendlines != null) {
        ...
        for (final Trendline trendline in cartesianSeries.trendlines!) {
          ...
          trendlineRenderer.name ??= (trendline.type == TrendlineType.movingAverage ? 'Moving average' : trendline.type.toString().substring(14)) +
        ...
        for (final TrendlineRenderer trendlineRenderer in seriesRendererDetails.trendlineRenderer) {
          ...
          trendlineRenderer.name = trendlineRenderer.name![0].toUpperCase() + trendlineRenderer.name!.substring(1);
          if (trendlineTypes[trendline.type.index] == 1 && trendlineRenderer.name![trendlineRenderer.name!.length - 1] == '0') {
            trendlineRenderer.name = trendlineRenderer.name!.substring(0, trendlineRenderer.name!.length - 2);
          }
        }
      }
      if (_stateProperties.renderingDetails.initialRender! ||
          (_stateProperties.renderingDetails.widgetNeedUpdate &&
              ...
              (_stateProperties.renderingDetails.oldDeviceOrientation == MediaQuery.of(context).orientation))) {
        if (seriesRendererDetails.oldSeries != null &&
            (_stateProperties.renderingDetails.legendRenderer.legend!.isVisible! ||
                _stateProperties.renderingDetails.legendToggleStates.isNotEmpty) &&
            (seriesRendererDetails.oldSeries!.isVisible == seriesRendererDetails.series.isVisible)) {
          legendCheck = true;
        } else {
          ...
          seriesRendererDetails.visible = _stateProperties.renderingDetails.initialRender!
              ? seriesRendererDetails.series.isVisible
              : seriesRendererDetails.visible ?? seriesRendererDetails.series.isVisible;
        }
      } else {
        legendCheck = true;
      }

      final SeriesRendererDetails seriesDetails = SeriesHelper.getSeriesRendererDetails(seriesRenderers[0]);
      final bool isFirstBarSeries = seriesDetails.series.toString().contains('Bar') && !seriesDetails.series.toString().contains('ErrorBar');
      final bool isMultipleBarSeries =
          seriesRendererDetails.series.toString().contains('Bar') && !seriesRendererDetails.series.toString().contains('ErrorBar');
      if (i == 0 || (!isFirstBarSeries && !isMultipleBarSeries) || (isFirstBarSeries && isMultipleBarSeries)) {
        visibleSeriesRenderers.add(seriesRenderers[i]);
        if (!_stateProperties.renderingDetails.initialRender! && _stateProperties.oldSeriesVisible.isNotEmpty && i < visibleSeriesRenderers.length) {
          if (i < visibleSeriesRenderers.length && i < _stateProperties.oldSeriesVisible.length) {
            _stateProperties.oldSeriesVisible[i] = SeriesHelper.getSeriesRendererDetails(visibleSeriesRenderers[i]).visible;
          }
        }
        if (_stateProperties.renderingDetails.legendToggleStates.isNotEmpty) {
          final SeriesRendererDetails visibleSeriesRendererDetails =
              SeriesHelper.getSeriesRendererDetails(visibleSeriesRenderers[visibleSeriesRenderers.length - 1]);
          for (final LegendRenderContext toggledLegendContext in _stateProperties.renderingDetails.legendToggleStates) {
            if (toggledLegendContext.seriesIndex == visibleSeriesRendererDetails.seriesIndex &&
                toggledLegendContext.series == visibleSeriesRendererDetails.oldSeries) {
              toggledLegendContext.series = visibleSeriesRendererDetails.series;
              toggledLegendContext.seriesRenderer = visibleSeriesRendererDetails;
              if (toggledLegendContext.isTrendline! &&
                  ...
                  toggledLegendContext.trendline == visibleSeriesRendererDetails.oldSeries?.trendlines![toggledLegendContext.trendlineIndex!]) {
                toggledLegendContext.trendline = visibleSeriesRendererDetails.series.trendlines![toggledLegendContext.trendlineIndex!];
              }
            }
          }
        }
        if (legendCheck) {
          ...
          final SeriesRendererDetails visibleSeriesDetails = SeriesHelper.getSeriesRendererDetails(visibleSeriesRenderers[index]);
          final String? legendItemText = visibleSeriesDetails.series.legendItemText;
          final String? legendText = _stateProperties.chart.legend.legendItemBuilder != null
              ? visibleSeriesDetails.seriesName
              : visibleSeriesDetails.series.isVisibleInLegend == true &&
                      _stateProperties.renderingDetails.chartLegend.legendCollections != null &&
                      _stateProperties.renderingDetails.chartLegend.legendCollections!.isNotEmpty
          ...
          final SeriesRendererDetails seriesDetails =
              SeriesHelper.getSeriesRendererDetails(_stateProperties.chartSeries.visibleSeriesRenderers[visibleSeriesRenderers.length - 1]);
          seriesDetails.visible = _checkWithLegendToggleState(
              visibleSeriesRenderers.length - 1,
              SeriesHelper.getSeriesRendererDetails(visibleSeriesRenderers[visibleSeriesRenderers.length - 1]).series,
              legendText ?? legendItemText ?? seriesName ?? 'Series $index');
        }
        // ignore: unnecessary_type_check
        final CartesianSeriesRenderer? cSeriesRenderer =
            _stateProperties.chartSeries.visibleSeriesRenderers[visibleSeriesRenderers.length - 1] is CartesianSeriesRenderer
                ? _stateProperties.chartSeries.visibleSeriesRenderers[visibleSeriesRenderers.length - 1]
                : null;
        final SeriesRendererDetails cSeriesDetails = SeriesHelper.getSeriesRendererDetails(cSeriesRenderer!);
        // ignore: unnecessary_null_comparison
        if (cSeriesDetails.series != null && cSeriesDetails.series.trendlines != null) {
          Trendline? trendline;
          TrendlineRenderer trendlineRenderer;
          for (int j = 0; j < cSeriesDetails.series.trendlines!.length; j++) {
            ...
            trendlineRenderer.visible = _checkWithTrendlineLegendToggleState(
                    visibleSeriesRenderers.length - 1, cSeriesDetails.series, j, trendline, trendlineRenderer.name!) &&
    ...
    _stateProperties.chartSeries.visibleSeriesRenderers = visibleSeriesRenderers;
    ...
    if (_stateProperties.chart.indicators.isNotEmpty) {
      ...
      for (int i = 0; i < _stateProperties.chart.indicators.length; i++) {
        technicalIndicatorRenderer = TechnicalIndicatorsRenderer(_stateProperties.chart.indicators[i], _stateProperties);
        _stateProperties.technicalIndicatorRenderer.add(technicalIndicatorRenderer);
        technicalIndicatorRenderer.visible = _stateProperties.renderingDetails.initialRender!
            ? _stateProperties.chart.indicators[i].isVisible
            : _checkIndicatorLegendToggleState(
                visibleSeriesRenderers.length + i, technicalIndicatorRenderer.visible ?? _stateProperties.chart.indicators[i].isVisible);
      }
    }
  ...
  CartesianChartPoint<dynamic> _getCrosshairChartPoint(CartesianStateProperties stateProperties) {
    CartesianChartPoint<dynamic> crosshairChartPoint = CartesianChartPoint<dynamic>();
    final CrosshairBehaviorRenderer crosshairBehaviorRenderer = stateProperties.crosshairBehaviorRenderer;
    final CrosshairRenderingDetails crosshairRenderingDetails = CrosshairHelper.getRenderingDetails(crosshairBehaviorRenderer);
    if (stateProperties.renderingDetails.oldDeviceOrientation != stateProperties.renderingDetails.deviceOrientation ||
      ...
      if (crosshairRenderingDetails.position != null && stateProperties.chart.crosshairBehavior.enable) {
        crosshairChartPoint = calculatePixelToPoint(crosshairRenderingDetails.position!, stateProperties.seriesRenderers.first);
      }
    }
  ...
  void _validateStateMaintenance(CartesianStateProperties stateProperties, CartesianChartPoint<dynamic> point) {
    final TrackballBehaviorRenderer trackballBehaviorRenderer = stateProperties.trackballBehaviorRenderer;
    final TrackballRenderingDetails trackballRenderingDetails = TrackballHelper.getRenderingDetails(trackballBehaviorRenderer);
    ...
    if (stateProperties.renderingDetails.oldDeviceOrientation != stateProperties.renderingDetails.deviceOrientation ||
        stateProperties.renderingDetails.didSizeChange) {
      if (trackballRenderingDetails.chartPointInfo.isNotEmpty && stateProperties.chart.trackballBehavior.enable) {
        stateProperties.isTrackballOrientationChanged = true;
        trackballRenderingDetails.internalShowByIndex(trackballRenderingDetails.chartPointInfo[0].dataPointIndex!);
      }
      if (crosshairRenderingDetails.position != null && stateProperties.chart.crosshairBehavior.enable) {
        stateProperties.isCrosshairOrientationChanged = true;
        crosshairOffset = calculatePointToPixel(point, stateProperties.seriesRenderers.first);
        crosshairRenderingDetails.internalShow(crosshairOffset.dx, crosshairOffset.dy, 'pixel');
      }
      if (tooltipRenderingDetails.showLocation != null &&
        ...
        tooltipRenderingDetails.internalShowByIndex(tooltipRenderingDetails.currentSeriesDetails.seriesIndex, tooltipRenderingDetails.pointIndex!);
      }
    }
  }
  ...
  LegendRenderContext? _getLegendItemCollection(int index) {
    for (final LegendRenderContext legendContext in _stateProperties.renderingDetails.chartLegend.legendCollections!) {
  ...
  bool _checkIndicatorLegendToggleState(int seriesIndex, bool seriesVisible) {
    ...
    if (widget.legend.legendItemBuilder != null) {
      final List<MeasureWidgetContext> legendToggles = _stateProperties.renderingDetails.legendToggleTemplateStates;
      ...
      if (_stateProperties.renderingDetails.legendToggleStates.isNotEmpty) {
        for (int j = 0; j < _stateProperties.renderingDetails.legendToggleStates.length; j++) {
          final LegendRenderContext legendRenderContext = _stateProperties.renderingDetails.legendToggleStates[j];
  ...
  bool _checkWithLegendToggleState(int seriesIndex, ChartSeries<dynamic, dynamic> series, String text) {
    ...
    if (_stateProperties.chart.legend.legendItemBuilder != null) {
      ...
      if (_stateProperties.renderingDetails.legendToggleStates.isNotEmpty) {
        for (int j = 0; j < _stateProperties.renderingDetails.legendToggleStates.length; j++) {
          final LegendRenderContext legendRenderContext = _stateProperties.renderingDetails.legendToggleStates[j];
          if (seriesIndex == legendRenderContext.seriesIndex && !legendRenderContext.isTrendline! && legendRenderContext.series == series) {
...
class ContainerArea extends StatelessWidget {
  ...
  TrackballRenderingDetails get trackballRenderingDetails => TrackballHelper.getRenderingDetails(_stateProperties.trackballBehaviorRenderer);

  @override
  Widget build(BuildContext context) {
    final ZoomingBehaviorDetails zoomingBehaviorDetails = ZoomPanBehaviorHelper.getRenderingDetails(_stateProperties.zoomPanBehaviorRenderer);

    //this boolean prohibits both x and y scrolls for the parent widget
    final bool isXYPanMode = (chart.crosshairBehavior.enable && chart.crosshairBehavior.activationMode == ActivationMode.singleTap) ||
        ((chart.zoomPanBehavior.enablePinching || chart.zoomPanBehavior.enablePanning) && chart.zoomPanBehavior.zoomMode == ZoomMode.xy);

    final bool requireInvertedAxis = _stateProperties.seriesRenderers.isNotEmpty
        ? (chart.isTransposed ^ getSeriesType(_stateProperties.seriesRenderers[0]).toLowerCase().contains('bar'))
        : chart.isTransposed;

    //this boolean prohibits x scrolls for the parent widget
    final bool isXPan =
        (chart.trackballBehavior.enable && !requireInvertedAxis && chart.trackballBehavior.activationMode == ActivationMode.singleTap) ||
            chart.onPlotAreaSwipe != null ||
            (!requireInvertedAxis && chart.loadMoreIndicatorBuilder != null) ||
            (chart.zoomPanBehavior.enablePanning && chart.zoomPanBehavior.zoomMode == ZoomMode.x);

    //this boolean prohibits y scrolls for the parent widget
    final bool isYPan =
        (chart.trackballBehavior.enable && requireInvertedAxis && chart.trackballBehavior.activationMode == ActivationMode.singleTap) ||
            (requireInvertedAxis && chart.loadMoreIndicatorBuilder != null) ||
            (chart.zoomPanBehavior.enablePanning && chart.zoomPanBehavior.zoomMode == ZoomMode.y);
    return LayoutBuilder(builder: (BuildContext context, BoxConstraints constraints) {
      return Container(
          ...
          child: MouseRegion(
              ...
              onHover: (PointerEvent event) => _enableMouseHover ? _performMouseHover(event) : null,
              ...
              child: Listener(
                  ...
                  onPointerSignal: (PointerSignalEvent event) {
                    if (_stateProperties.chartState.mounted && event is PointerScrollEvent) {
                  ...
                  child: GestureDetector(
                      ...
                      onTap: () {
                        if (_stateProperties.isArea ?? false) {
                          if (zoomingBehaviorDetails.isPinching != true) {
                            zoomingBehaviorDetails.canPerformSelection = false;
                            if (chart.zoomPanBehavior.enableSelectionZooming && zoomingBehaviorDetails.zoomingRect.width != 0) {
                              zoomingBehaviorDetails.doSelectionZooming(zoomingBehaviorDetails.zoomingRect, isArea: true);
                        ...
                        if (_stateProperties.chartState.mounted &&
                            _stateProperties.chartSeries.visibleSeriesRenderers.isNotEmpty &&
                            _touchPosition != null &&
                          ...
                          final SeriesRendererDetails selectionSeriesDetails = SeriesHelper.getSeriesRendererDetails(selectionseriesRenderer);
                          if (selectionSeriesDetails.visible ?? false) {
                            final SelectionBehaviorRenderer? selectionBehaviorRenderer = selectionSeriesDetails.selectionBehaviorRenderer;
                            final SelectionDetails? selectionDetails = SelectionHelper.getRenderingDetails(selectionBehaviorRenderer!);
                            if (selectionSeriesDetails.isSelectionEnable == true &&
                                selectionDetails?.selectionRenderer != null &&
                                selectionSeriesDetails.isOuterRegion == false) {
                              selectionDetails?.selectionRenderer?.seriesRendererDetails = selectionSeriesDetails;
                      ...
                      onTapUp: (TapUpDetails details) {
                        if (_stateProperties.chartState.mounted) {
                          final Offset position = renderBox.globalToLocal(details.globalPosition);
                          final List<CartesianSeriesRenderer> visibleSeriesRenderer = _stateProperties.chartSeries.visibleSeriesRenderers;
                          final CartesianSeriesRenderer? cartesianSeriesRenderer = _findSeries(position);
                          if (cartesianSeriesRenderer != null &&
                              SeriesHelper.getSeriesRendererDetails(cartesianSeriesRenderer).series.onPointTap != null) {
                            calculatePointSeriesIndex(chart, _stateProperties, position, null, ActivationMode.singleTap);
                          }
                          if (chart.onAxisLabelTapped != null) {
                            _triggerAxisLabelEvent(position);
                          }
                          if (chart.onDataLabelTapped != null) {
                            triggerDataLabelEvent(chart, visibleSeriesRenderer, position);
                          }
                        }
                      },
                      onDoubleTap: () {
  ...
  Widget _initializeChart(BoxConstraints constraints, BuildContext context) {
    ...
    _calculateBounds();
    return Container(decoration: const BoxDecoration(color: Colors.transparent), child: _renderWidgets(constraints, context));
  }
  ...
  void _calculateContainerSize(BoxConstraints constraints) {
    final double width = constraints.maxWidth;
    final double height = constraints.maxHeight;
    _stateProperties.renderingDetails.chartContainerRect = Rect.fromLTWH(0, 0, width, height);
  }
  ...
  void _calculateTrendlineRegion(CartesianStateProperties stateProperties, XyDataSeriesRenderer seriesRenderer) {
    final SeriesRendererDetails seriesRendererDetails = SeriesHelper.getSeriesRendererDetails(seriesRenderer);
    if (seriesRendererDetails.series.trendlines != null) {
      ...
      for (int i = 0; i < seriesRendererDetails.series.trendlines!.length; i++) {
        ...
        if (trendlineRenderer.isNeedRender) {
          trendlineRenderer.calculateTrendlinePoints(seriesRendererDetails, stateProperties);
        }
      }
    }
  }
  ...
  Widget _renderWidgets(BoxConstraints constraints, BuildContext context) {
    ...
    // ignore: avoid_unnecessary_containers
    return Container(child: Stack(textDirection: TextDirection.ltr, children: _stateProperties.renderingDetails.chartWidgets!));
  }

  void _bindLoadMoreIndicatorWidget() {
    _stateProperties.renderingDetails.chartWidgets!.add(StatefulBuilder(builder: (BuildContext context, StateSetter stateSetter) {
      Widget renderWidget;
      _stateProperties.loadMoreViewStateSetter = stateSetter;
      renderWidget = _stateProperties.isLoadMoreIndicator
          ? Center(child: _stateProperties.chart.loadMoreIndicatorBuilder!(context, _stateProperties.swipeDirection))
  ...
  void _bindPlotBandWidgets(bool shouldRenderAboveSeries) {
    _stateProperties.renderingDetails.chartWidgets!.add(RepaintBoundary(
        child: CustomPaint(painter: getPlotBandPainter(stateProperties: _stateProperties, shouldRenderAboveSeries: shouldRenderAboveSeries))));
  }

  void _bindTrendlineWidget() {
    bool isTrendline = false;
    final Map<String, Animation<double>> trendlineAnimations = <String, Animation<double>>{};
    for (int i = 0; i < _stateProperties.chartSeries.visibleSeriesRenderers.length; i++) {
      _seriesRenderer = _stateProperties.chartSeries.visibleSeriesRenderers[i] as XyDataSeriesRenderer;
      final SeriesRendererDetails seriesRendererDetails = SeriesHelper.getSeriesRendererDetails(_seriesRenderer);
      _series = seriesRendererDetails.series;
      // ignore: unnecessary_null_comparison
      if (_seriesRenderer != null && seriesRendererDetails.visible! == true && _series.trendlines != null) {
        isTrendline = true;
        for (int j = 0; j < seriesRendererDetails.trendlineRenderer.length; j++) {
          final TrendlineRenderer trendlineRenderer = seriesRendererDetails.trendlineRenderer[j];
          ...
          if (trendline.animationDuration > 0 &&
              (_renderingDetails.oldDeviceOrientation == null ||
                  _stateProperties.renderingDetails.oldDeviceOrientation == _stateProperties.renderingDetails.deviceOrientation) &&
              seriesRendererDetails.needsAnimation == true &&
              seriesRendererDetails.oldSeries == null) {
            final int totalAnimationDuration = trendline.animationDuration.toInt() + trendline.animationDelay!.toInt();
            trendlineRenderer.animationController.duration = Duration(milliseconds: totalAnimationDuration);
            const double maxSeriesInterval = 0.8;
            double minSeriesInterval = 0.1;
            minSeriesInterval =
                trendline.animationDelay!.toInt() / totalAnimationDuration * (maxSeriesInterval - minSeriesInterval) + minSeriesInterval;
            trendlineAnimations['$i-$j'] = Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(
  ...
  void _bindDataLabelWidgets() {
    _stateProperties.renderDataLabel = DataLabelRenderer(stateProperties: _stateProperties, show: _stateProperties.renderingDetails.animateCompleted);
    _stateProperties.renderingDetails.chartWidgets!.add(_stateProperties.renderDataLabel!);
  }
  ...
  void _renderTemplates() {
    ...
    if (chart.annotations != null && chart.annotations!.isNotEmpty) {
      for (int i = 0; i < chart.annotations!.length; i++) {
        ...
        final ChartLocation location = getAnnotationLocation(annotation, _stateProperties);
    ...
    if (_renderingDetails.templates.isNotEmpty) {
      final int templateLength = _stateProperties.renderingDetails.templates.length;
      for (int i = 0; i < _stateProperties.renderingDetails.templates.length; i++) {
        final ChartTemplateInfo templateInfo = _stateProperties.renderingDetails.templates[i];
        _stateProperties.renderingDetails.chartWidgets!
            .add(RenderTemplate(template: templateInfo, templateIndex: i, templateLength: templateLength, stateProperties: _stateProperties));
      }
    }
  }
  ...
  void _renderDataLabelTemplates() {
    ...
    _stateProperties.renderingDetails.dataLabelTemplateRegions = <Rect>[];
    for (int i = 0; i < _stateProperties.chartSeries.visibleSeriesRenderers.length; i++) {
      final CartesianSeriesRenderer seriesRenderer = _stateProperties.chartSeries.visibleSeriesRenderers[i];
      final SeriesRendererDetails seriesRendererDetails = SeriesHelper.getSeriesRendererDetails(seriesRenderer);
      ...
      if (series.dataLabelSettings.isVisible && seriesRendererDetails.visible! == true) {
        for (int j = 0; j < seriesRendererDetails.dataPoints.length; j++) {
          point = dataPoints[j];
          if (point.isVisible && !point.isGap && withInRange(point.xValue, seriesRendererDetails.xAxisDetails!.visibleRange!)) {
            labelWidget = (series.dataLabelSettings.builder != null)
                ? series.dataLabelSettings.builder!(series.dataSource[point.overallDataPointIndex!], point, series, point.overallDataPointIndex!, i)
                : null;
            if (labelWidget != null) {
              final String seriesType = seriesRendererDetails.seriesType;
              final List<num> dataLabelTemplateYValues =
                  (seriesType.contains('range') || (seriesType.contains('hilo') && !seriesType.contains('hiloopenclose')))
                      ? <num>[point.low as num, point.high as num]
                      : (seriesType.contains('candle') || seriesType.contains('hiloopenclose'))
                          ? <num>[point.low as num, point.high as num, point.open as num, point.close as num]
                          : seriesType.contains('box')
                              ? <num>[point.minimum!]
                              : <num>[point.y as num];
              for (int k = 0; k < dataLabelTemplateYValues.length; k++) {
                padding = (k == 0 && dataLabelTemplateYValues.length > 1 && !_stateProperties.requireInvertedAxis) ? 20 : 0;
                final ChartLocation location = calculatePoint(point.xValue, dataLabelTemplateYValues[k], seriesRendererDetails.xAxisDetails!,
                    seriesRendererDetails.yAxisDetails!, _stateProperties.requireInvertedAxis, series, _stateProperties.chartAxis.axisClipRect);
    ...
    for (int i = 0; i < _stateProperties.chartSeries.visibleSeriesRenderers.length; i++) {
      ...
      if (seriesRendererDetails.isIndicator == true) {
        ...
        if (_seriesRenderer is XyDataSeriesRenderer) {
          seriesRendererDetails.animationController = AnimationController(vsync: _stateProperties.chartState)
      ...
      if (_seriesRenderer != null && seriesRendererDetails.visible! == true) {
        ...
        seriesRendererDetails.selectionBehaviorRenderer = SelectionBehaviorRenderer(selectionBehavior, chart, _stateProperties);
        final SelectionBehaviorRenderer? selectionBehaviorRenderer = seriesRendererDetails.selectionBehaviorRenderer;
        SelectionHelper.setSelectionBehaviorRenderer(_series.selectionBehavior, selectionBehaviorRenderer!);
        // ignore: unnecessary_null_comparison
        if (selectionBehaviorRenderer != null) {
          ...
          selectionDetails.selectionRenderer!.stateProperties = _stateProperties;
          selectionDetails.selectionRenderer!.seriesRendererDetails = seriesRendererDetails;
          _series = seriesRendererDetails.series;
          if (selectionBehavior.selectionController != null) {
            selectionDetails.selectRange();
          }
          selectionDetails.selectionRenderer!.selectedSegments = _stateProperties.selectedSegments;
          selectionDetails.selectionRenderer!.unselectedSegments = _stateProperties.unselectedSegments;
          ...
          if (_series.initialSelectedDataIndexes != null &&
              ...
              seriesRendererDetails.oldSelectedIndexes!.length == _series.initialSelectedDataIndexes!.length) {
            for (final int index in _series.initialSelectedDataIndexes!) {
              isSelecetedIndexesUpdated = seriesRendererDetails.oldSelectedIndexes!.contains(index) == false;
              if (isSelecetedIndexesUpdated) {
                break;
              }
            }
          } else {
            isSelecetedIndexesUpdated = _series.initialSelectedDataIndexes!.isNotEmpty;
          }
          int totalSelectedSegment = 0;
          int? selectedSeriesIndex;
          if (selectionBehavior.enable == true && _stateProperties.selectedSegments.isNotEmpty && _stateProperties.unselectedSegments.isNotEmpty) {
            for (int j = 0; j < _stateProperties.selectedSegments.length; j++) {
              final SegmentProperties segmentProperties = SegmentHelper.getSegmentProperties(_stateProperties.selectedSegments[j]);
              if (segmentProperties.seriesIndex == i) {
                totalSelectedSegment += 1;
                selectedSeriesIndex = segmentProperties.seriesIndex;
              }
            }
            for (int k = 0; k < _stateProperties.unselectedSegments.length; k++) {
              if (SegmentHelper.getSegmentProperties(_stateProperties.unselectedSegments[k]).seriesIndex == i) {
                totalSelectedSegment += 1;
              }
            }
          }
              ...
              (isSelecetedIndexesUpdated ||
                  (!_renderingDetails.initialRender! &&
                      (totalSelectedSegment != 0 &&
                          (totalSelectedSegment < SeriesHelper.getSeriesRendererDetails(_stateProperties.seriesRenderers[i]).dataPoints.length))))) {
            int segmentLength = seriesRendererDetails.dataPoints.length;

            if (isLineTypeSeries(seriesRendererDetails.seriesType) || seriesRendererDetails.seriesType.contains('boxandwhisker') == true) {
              segmentLength = seriesRendererDetails.dataPoints.length - 1;
            }

            for (int j = 0; j < segmentLength; j++) {
              final ChartSegment segment = ColumnSegment();
              SegmentHelper.setSegmentProperties(segment, SegmentProperties(_stateProperties, segment));
              ...
              segmentProperties.currentPoint = seriesRendererDetails.dataPoints[j];
              ((_series.initialSelectedDataIndexes!.contains(segment.currentSegmentIndex) && isSelecetedIndexesUpdated) ||
                      chart.selectionType == SelectionType.series && selectedSeriesIndex == i)
                  ? SelectionHelper.getRenderingDetails(selectionBehaviorRenderer).selectionRenderer!.selectedSegments.add(segment)
                  : SelectionHelper.getRenderingDetails(selectionBehaviorRenderer).selectionRenderer!.unselectedSegments!.add(segment);
            }
        ...
        if (seriesRendererDetails.animationController != null &&
            ...
            (_renderingDetails.initialRender! ||
                _stateProperties.legendRefresh ||
                ((seriesType == 'column' || seriesType == 'bar') && _stateProperties.legendToggling) ||
                (!_stateProperties.legendToggling &&
                    seriesRendererDetails.needsAnimation == true &&
                    _stateProperties.renderingDetails.widgetNeedUpdate))) {
          if ((seriesType == 'column' || seriesType == 'bar') && _stateProperties.legendToggling) {
          ...
          seriesRendererDetails.seriesAnimation = Tween<double>(begin: 1, end: 1.0).animate(CurvedAnimation(
            parent: seriesRendererDetails.animationController,
            curve: const Interval(0.1, 0.8, curve: Curves.decelerate),
          ));
          seriesRendererDetails.seriesElementAnimation = Tween<double>(begin: 1, end: 1.0).animate(CurvedAnimation(
            parent: seriesRendererDetails.animationController,
      ...
      _stateProperties.renderingDetails.chartWidgets!.add(Container(
          child: RepaintBoundary(
              child: CustomPaint(
        painter: _getSeriesPainter(i, seriesRendererDetails.animationController, _seriesRenderer),
      ))));
    }
    _stateProperties.renderingDetails.chartWidgets!.add(Container(
        color: Colors.red,
        child: RepaintBoundary(
            child: CustomPaint(painter: ZoomRectPainter(stateProperties: _stateProperties, notifier: _stateProperties.repaintNotifiers['zoom'])))));
    _stateProperties.legendToggling = false;
  }
  ...
  void _bindAxisWidgets(String renderType) {
    ...
    if (_stateProperties.chartAxis.axisRenderersCollection != null &&
      ...
      final CartesianAxisWidget axisWidget = CartesianAxisWidget(stateProperties: _stateProperties, renderType: renderType);
      renderType == 'outside' ? _stateProperties.renderOutsideAxis = axisWidget : _stateProperties.renderInsideAxis = axisWidget;
  ...
  CartesianSeriesRenderer? _findSeries(Offset position) {
    ...
    outerLoop:
    for (int i = _stateProperties.chartSeries.visibleSeriesRenderers.length - 1; i >= 0; i--) {
      ...
      if (seriesType == 'column' ||
        ...
        for (int j = 0; j < dataPoints.length; j++) {
          if (dataPoints[j].region != null && dataPoints[j].region!.contains(position) == true) {
        ...
        selectionBehaviorRenderer = seriesRendererDetails.selectionBehaviorRenderer;
        bool isSelect = false;
        seriesRenderer = _stateProperties.chartSeries.visibleSeriesRenderers[i];
        for (int k = _stateProperties.chartSeries.visibleSeriesRenderers.length - 1; k >= 0; k--) {
          isSelect = seriesRendererDetails.isSelectionEnable == true &&
              ...
              (SelectionHelper.getRenderingDetails(selectionBehaviorRenderer!).selectionRenderer!.isSeriesContainsPoint(
                      SeriesHelper.getSeriesRendererDetails(_stateProperties.chartSeries.visibleSeriesRenderers[i]), position) ==
                  true);
          if (isSelect) {
  ...
  void _performPointerDown(PointerDownEvent event) {
    ...
    TooltipHelper.getRenderingDetails(_stateProperties.renderingDetails.tooltipBehaviorRenderer).isHovering = false;
    ...
    if (chart.zoomPanBehavior.enablePinching == true) {
      ...
      if (_stateProperties.touchStartPositions.length == 2) {
        for (int axisIndex = 0; axisIndex < _stateProperties.chartAxis.axisRenderersCollection.length; axisIndex++) {
          final ChartAxisRendererDetails axisDetails =
              AxisHelper.getAxisRendererDetails(_stateProperties.chartAxis.axisRenderersCollection[axisIndex]);

          if (chart.onZoomStart != null) {
            zoomStartArgs = bindZoomEvent(chart, axisDetails, chart.onZoomStart!);
    ...
    if (chart.trackballBehavior != null &&
        ...
        SeriesHelper.getSeriesRendererDetails(cartesianSeriesRenderer).series is! ErrorBarSeries) {
      if (chart.trackballBehavior.builder != null) {
        ...
        _stateProperties.trackballBehaviorRenderer.onTouchDown(position.dx, position.dy);
      }
    }
    // ignore: unnecessary_null_comparison
    if (chart.crosshairBehavior != null &&
        ...
        cartesianSeriesRenderer != null &&
        SeriesHelper.getSeriesRendererDetails(cartesianSeriesRenderer).series is! ErrorBarSeries) {
      _stateProperties.crosshairBehaviorRenderer.onTouchDown(position.dx, position.dy);
    }
  }
  ...
  void _performPointerMove(PointerMoveEvent event) {
    TooltipHelper.getRenderingDetails(_stateProperties.renderingDetails.tooltipBehaviorRenderer).isHovering = false;
    final ZoomingBehaviorDetails zoomingBehaviorDetails = ZoomPanBehaviorHelper.getRenderingDetails(_stateProperties.zoomPanBehaviorRenderer);
    if (chart.zoomPanBehavior.enablePinching == true && _stateProperties.touchStartPositions.length == 2) {
      zoomingBehaviorDetails.isPinching = true;
      ...
      if (_stateProperties.touchMovePositions.length == 2 && _stateProperties.touchStartPositions.length == 2) {
        ...
        zoomingBehaviorDetails.performPinchZooming(_stateProperties.touchStartPositions, _stateProperties.touchMovePositions);
      }
    }
  }
    ...
    if ((chart.trackballBehavior != null &&
        ...
        (chart.zoomPanBehavior != null &&
            ((chart.zoomPanBehavior.enableDoubleTapZooming ||
                ...
                !chart.trackballBehavior.shouldAlwaysShow))) {
      _stateProperties.trackballBehaviorRenderer.onTouchUp(position.dx, position.dy);
    ...
    if ((chart.crosshairBehavior != null &&
        ...
        (chart.zoomPanBehavior != null &&
            ((chart.zoomPanBehavior.enableDoubleTapZooming ||
                    ...
                    chart.zoomPanBehavior.enableSelectionZooming) &&
                !chart.crosshairBehavior.shouldAlwaysShow))) {
      _stateProperties.crosshairBehaviorRenderer.onTouchUp(position.dx, position.dy);
      CrosshairHelper.getRenderingDetails(_stateProperties.crosshairBehaviorRenderer).isLongPressActivated = false;
    }
    if (chart.tooltipBehavior.enable && chart.tooltipBehavior.activationMode == ActivationMode.singleTap || shouldShowAxisTooltip(_stateProperties)) {
      ...
      chart.tooltipBehavior.builder != null
          ...
          : _stateProperties.renderingDetails.tooltipBehaviorRenderer.onTouchUp(position.dx, position.dy);
    }
  }
  ...
  void _performPointerSignal(PointerScrollEvent event) {
    ...
    if (_mousePointerDetails != null) {
      final Offset position = renderBox.globalToLocal(event.position);
      if (chart.zoomPanBehavior.enableMouseWheelZooming && _stateProperties.chartAxis.axisClipRect.contains(position)) {
        ZoomPanBehaviorHelper.getRenderingDetails(_stateProperties.zoomPanBehaviorRenderer).performMouseWheelZooming(event, position.dx, position.dy);
      }
    }
  }
  ...
  void _calculatePinchZoomingArgs() {
    ...
    int axisIndex;
    for (axisIndex = 0; axisIndex < _stateProperties.chartAxis.axisRenderersCollection.length; axisIndex++) {
      final ChartAxisRendererDetails axisDetails = AxisHelper.getAxisRendererDetails(_stateProperties.chartAxis.axisRenderersCollection[axisIndex]);
    ...
    if (resetFlag) {
      for (int index = 0; index < _stateProperties.chartAxis.axisRenderersCollection.length; index++) {
        final ChartAxisRendererDetails axisDetails = AxisHelper.getAxisRendererDetails(_stateProperties.chartAxis.axisRenderersCollection[index]);
  ...
  void _performLongPressMoveUpdate(LongPressMoveUpdateDetails details) {
    ...
    if (zoomingBehaviorDetails.isPinching != true) {
      if (chart.zoomPanBehavior.enableSelectionZooming && position != null && _zoomStartPosition != null) {
        ...
        _stateProperties.zoomPanBehaviorRenderer.onDrawSelectionZoomRect(position.dx, position.dy, _zoomStartPosition!.dx, _zoomStartPosition!.dy);
      }
    }
    // ignore: unnecessary_null_comparison
    if (chart.trackballBehavior != null &&
        ...
        SeriesHelper.getSeriesRendererDetails(_findSeries(position)!).series is! ErrorBarSeries) {
      if (chart.trackballBehavior.activationMode == ActivationMode.singleTap) {
        chart.trackballBehavior.builder != null
            ? trackballRenderingDetails.showTemplateTrackball(position)
            : _stateProperties.trackballBehaviorRenderer.onTouchMove(position.dx, position.dy);
      }
      if (chart.trackballBehavior.activationMode == ActivationMode.longPress && trackballRenderingDetails.isLongPressActivated == true) {
        chart.trackballBehavior.builder != null
            ? trackballRenderingDetails.showTemplateTrackball(position)
            : _stateProperties.trackballBehaviorRenderer.onTouchMove(position.dx, position.dy);
      }
    ...
    if (chart.crosshairBehavior != null &&
      ...
      if (chart.crosshairBehavior.activationMode == ActivationMode.singleTap) {
        _stateProperties.crosshairBehaviorRenderer.onTouchMove(position.dx, position.dy);
        // ignore: unnecessary_null_comparison
      } else if ((chart.crosshairBehavior != null &&
              chart.crosshairBehavior.activationMode == ActivationMode.longPress &&
              CrosshairHelper.getRenderingDetails(_stateProperties.crosshairBehaviorRenderer).isLongPressActivated == true) &&
          !chart.zoomPanBehavior.enableSelectionZooming) {
        _stateProperties.crosshairBehaviorRenderer.onTouchMove(position.dx, position.dy);
      }
    }
  ...
  void _performLongPressEnd() {
    final ZoomingBehaviorDetails zoomingBehaviorDetails = ZoomPanBehaviorHelper.getRenderingDetails(_stateProperties.zoomPanBehaviorRenderer);
    if (_stateProperties.isArea ?? false) {
      _stateProperties.onAreaCallback?.call('bottom', _stateProperties.bottomStart, _stateProperties.bottomEnd);
      return;
    }
    ...
    if (zoomingBehaviorDetails.isPinching != true) {
      zoomingBehaviorDetails.canPerformSelection = false;
      if (chart.zoomPanBehavior.enableSelectionZooming && zoomingBehaviorDetails.zoomingRect.width != 0) {
        zoomingBehaviorDetails.doSelectionZooming(zoomingBehaviorDetails.zoomingRect);
        if (zoomingBehaviorDetails.canPerformSelection != true) {
          zoomingBehaviorDetails.zoomingRect = Rect.zero;
        }
      }
  ...
  void _performPanDown(DragDownDetails details) {
    _stateProperties.startOffset = renderBox.globalToLocal(details.globalPosition);
    final ZoomingBehaviorDetails zoomingBehaviorDetails = ZoomPanBehaviorHelper.getRenderingDetails(_stateProperties.zoomPanBehaviorRenderer);
    if (zoomingBehaviorDetails.isPinching != true) {
  ...
  void _performLongPress() {
    ...
    if (_tapDownDetails != null) {
      ...
      final CartesianSeriesRenderer? cartesianSeriesRenderer = _findSeries(position);
      if (cartesianSeriesRenderer != null && SeriesHelper.getSeriesRendererDetails(cartesianSeriesRenderer).series.onPointLongPress != null) {
        calculatePointSeriesIndex(chart, _stateProperties, position, null, ActivationMode.longPress);
      }
      if (chart.tooltipBehavior.enable && chart.tooltipBehavior.activationMode == ActivationMode.longPress ||
        ...
        chart.tooltipBehavior.builder != null
            ? tooltipRenderingDetails.showTemplateTooltip(position)
            : _stateProperties.renderingDetails.tooltipBehaviorRenderer.onLongPress(position.dx, position.dy);
      }
    }
    // ignore: unnecessary_null_comparison
    if (_stateProperties.chartSeries.visibleSeriesRenderers != null && position != null && chart.selectionGesture == ActivationMode.longPress) {
      final CartesianSeriesRenderer selectionSeriesRenderer = _findSeries(position)!;
      final SeriesRendererDetails seriesRendererDetails = SeriesHelper.getSeriesRendererDetails(selectionSeriesRenderer);
      final SelectionBehaviorRenderer selectionBehaviorRenderer = seriesRendererDetails.selectionBehaviorRenderer!;
      SelectionHelper.getRenderingDetails(selectionBehaviorRenderer).selectionRenderer!.seriesRendererDetails = seriesRendererDetails;
  ...
  void _performDoubleTap() {
    if (_tapDownDetails != null) {
      ...
      final CartesianSeriesRenderer? cartesianSeriesRenderer = _findSeries(position);
      if (cartesianSeriesRenderer != null && SeriesHelper.getSeriesRendererDetails(cartesianSeriesRenderer).series.onPointDoubleTap != null) {
        calculatePointSeriesIndex(chart, _stateProperties, position, null, ActivationMode.doubleTap);
      }
      // ignore: unnecessary_null_comparison
      if (chart.trackballBehavior != null &&
          chart.trackballBehavior.enable &&
        ...
        chart.trackballBehavior.builder != null
            ? trackballRenderingDetails.showTemplateTrackball(position)
            : _stateProperties.trackballBehaviorRenderer.onDoubleTap(position.dx, position.dy);
      ...
      if (chart.crosshairBehavior != null &&
          ...
          SeriesHelper.getSeriesRendererDetails(cartesianSeriesRenderer).series is! ErrorBarSeries &&
          chart.crosshairBehavior.activationMode == ActivationMode.doubleTap) {
        _stateProperties.crosshairBehaviorRenderer.onDoubleTap(position.dx, position.dy);
        _stateProperties.enableDoubleTap = true;
        _stateProperties.isTouchUp = true;
        _stateProperties.crosshairBehaviorRenderer.onTouchUp(position.dx, position.dy);
        _stateProperties.isTouchUp = false;
      ...
      if (chart.tooltipBehavior.enable && chart.tooltipBehavior.activationMode == ActivationMode.doubleTap ||
        ...
        chart.tooltipBehavior.builder != null
            ? tooltipRenderingDetails.showTemplateTooltip(position)
            : _stateProperties.renderingDetails.tooltipBehaviorRenderer.onDoubleTap(position.dx, position.dy);
      }
      // ignore: unnecessary_null_comparison
      if (_stateProperties.chartSeries.visibleSeriesRenderers != null && chart.selectionGesture == ActivationMode.doubleTap) {
        final CartesianSeriesRenderer selectionSeriesRenderer = _findSeries(position)!;
        final SeriesRendererDetails seriesRendererDetails = SeriesHelper.getSeriesRendererDetails(selectionSeriesRenderer);
        final SelectionBehaviorRenderer selectionBehaviorRenderer = seriesRendererDetails.selectionBehaviorRenderer!;
        SelectionHelper.getRenderingDetails(selectionBehaviorRenderer).selectionRenderer!.seriesRendererDetails = seriesRendererDetails;
    ...
    if (chart.zoomPanBehavior.enableDoubleTapZooming == true) {
      ...
      if (position != null) {
        _stateProperties.zoomPanBehaviorRenderer
            .onDoubleTap(position.dx, position.dy, ZoomPanBehaviorHelper.getRenderingDetails(_stateProperties.zoomPanBehaviorRenderer).zoomFactor);
      }
    }
  }
  ...
  void _performPanUpdate(DragUpdateDetails details) {
    if (_stateProperties.chartState.mounted) {
      Offset? position;
      _stateProperties.currentPosition = renderBox.globalToLocal(details.globalPosition);
      final ZoomingBehaviorDetails zoomingBehaviorDetails = ZoomPanBehaviorHelper.getRenderingDetails(_stateProperties.zoomPanBehaviorRenderer);
      if (zoomingBehaviorDetails.isPinching != true) {
        ...
        if ((zoomingBehaviorDetails.isPanning ?? false) &&
          ...
          _stateProperties.zoomPanBehaviorRenderer.onPan(position.dx, position.dy);
        }
        zoomingBehaviorDetails.previousMovedPosition = position;
      }
      final bool panInProgress = chart.zoomPanBehavior.enablePanning && zoomingBehaviorDetails.previousMovedPosition != null;
      // ignore: unnecessary_null_comparison
      if (chart.trackballBehavior != null &&
          chart.trackballBehavior.enable &&
          position != null &&
  ...
  void _performPanEnd(DragEndDetails details) {
    if (_stateProperties.chartState.mounted) {
      ...
      if (chart.trackballBehavior.enable &&
          ...
          chart.trackballBehavior.activationMode != ActivationMode.doubleTap &&
          _touchPosition != null &&
          SeriesHelper.getSeriesRendererDetails(_findSeries(_touchPosition!)!).series is! ErrorBarSeries) {
        _stateProperties.isTouchUp = true;
        _stateProperties.trackballBehaviorRenderer.onTouchUp(_touchPosition!.dx, _touchPosition!.dy);
        _stateProperties.isTouchUp = false;
        trackballRenderingDetails.isLongPressActivated = false;
      }
      if (chart.crosshairBehavior.enable &&
          ...
          chart.crosshairBehavior.activationMode != ActivationMode.doubleTap) {
        _stateProperties.isTouchUp = true;
        _stateProperties.crosshairBehaviorRenderer.onTouchUp(_touchPosition!.dx, _touchPosition!.dy);
        _stateProperties.isTouchUp = false;
        CrosshairHelper.getRenderingDetails(_stateProperties.crosshairBehaviorRenderer).isLongPressActivated = false;
      }
      ...
      if (chart.onPlotAreaSwipe != null &&
          ...
          _stateProperties.chartAxis.axisClipRect.contains(_stateProperties.startOffset!) &&
          _stateProperties.chartAxis.axisClipRect.contains(_stateProperties.currentPosition!)) {
        //swipe configuration options
        const double swipeMaxDistanceThreshold = 50.0;
        final double swipeMinDisplacement =
            (_stateProperties.requireInvertedAxis ? _stateProperties.chartAxis.axisClipRect.height : _stateProperties.chartAxis.axisClipRect.width) *
                0.1;
        final double swipeMinVelocity = _stateProperties.pointerDeviceKind == PointerDeviceKind.mouse ? 0.0 : 240;
        ChartSwipeDirection swipeDirection;

        final double dx = (_stateProperties.currentPosition!.dx - _stateProperties.startOffset!.dx).abs();
        final double dy = (_stateProperties.currentPosition!.dy - _stateProperties.startOffset!.dy).abs();
        ...
        if (_stateProperties.requireInvertedAxis &&
          ...
          ///vertical
          swipeDirection = _stateProperties.pointerDeviceKind == PointerDeviceKind.mouse
              ? (_stateProperties.currentPosition!.dy > _stateProperties.startOffset!.dy ? ChartSwipeDirection.end : ChartSwipeDirection.start)
              : (velocity < 0 ? ChartSwipeDirection.start : ChartSwipeDirection.end);
          ...
          ///horizontal
          swipeDirection = _stateProperties.pointerDeviceKind == PointerDeviceKind.mouse
              ? (_stateProperties.currentPosition!.dx > _stateProperties.startOffset!.dx ? ChartSwipeDirection.start : ChartSwipeDirection.end)
              : (velocity > 0 ? ChartSwipeDirection.start : ChartSwipeDirection.end);
          chart.onPlotAreaSwipe!(swipeDirection);
        }
      }
      ...
      ///Load More feature
      if (chart.loadMoreIndicatorBuilder != null && _stateProperties.startOffset != null && _stateProperties.currentPosition != null) {
        final bool verticallyDragging = (_stateProperties.currentPosition!.dy - _stateProperties.startOffset!.dy).abs() >
            (_stateProperties.currentPosition!.dx - _stateProperties.startOffset!.dx).abs();
        if ((!verticallyDragging && !_stateProperties.requireInvertedAxis) || (verticallyDragging && _stateProperties.requireInvertedAxis)) {
          bool loadMore = false;
          final ChartAxisRendererDetails primaryXAxisDetails = _stateProperties.chartAxis.primaryXAxisDetails;
          // Here, direction is set accordingly based on the axis transposed value
          // and primary x-axis inversed value.
          final ChartSwipeDirection direction = _stateProperties.requireInvertedAxis
              ? (_stateProperties.currentPosition!.dy > _stateProperties.startOffset!.dy
                  ...
                  : primaryXAxisDetails.axis.isInversed
                      ? ChartSwipeDirection.end
                      : ChartSwipeDirection.start)
              : (_stateProperties.currentPosition!.dx > _stateProperties.startOffset!.dx
                  ? primaryXAxisDetails.axis.isInversed
                      ? ChartSwipeDirection.end
                      : ChartSwipeDirection.start
                  : primaryXAxisDetails.axis.isInversed
          ...
          for (int axisIndex = 0; axisIndex < _stateProperties.chartAxis.axisRenderersCollection.length; axisIndex++) {
            final ChartAxisRendererDetails axisDetails =
                AxisHelper.getAxisRendererDetails(_stateProperties.chartAxis.axisRenderersCollection[axisIndex]);
            if (((!verticallyDragging && axisDetails.orientation == AxisOrientation.horizontal) ||
                    (verticallyDragging && axisDetails.orientation == AxisOrientation.vertical)) &&
                axisDetails.actualRange != null &&
                ((axisDetails.actualRange!.minimum.round() == axisDetails.visibleRange!.minimum.round() && direction == ChartSwipeDirection.start) ||
                    (axisDetails.actualRange!.maximum.round() == axisDetails.visibleRange!.maximum.round() &&
  ...
  void _performMouseHover(PointerEvent event) {
    if (_stateProperties.chartState.mounted) {
      ...
      if ((chart.tooltipBehavior.enable && chart.tooltipBehavior.activationMode == ActivationMode.singleTap) ||
        ...
        chart.tooltipBehavior.builder != null
            ? tooltipRenderingDetails.showTemplateTooltip(position)
            : _stateProperties.renderingDetails.tooltipBehaviorRenderer.onEnter(position.dx, position.dy);
      }
      if (chart.trackballBehavior.enable &&
          cartesianSeriesRenderer != null &&
          SeriesHelper.getSeriesRendererDetails(cartesianSeriesRenderer).series is! ErrorBarSeries &&
        ...
        chart.trackballBehavior.builder != null
            ? trackballRenderingDetails.showTemplateTrackball(position)
            : _stateProperties.trackballBehaviorRenderer.onEnter(position.dx, position.dy);
      }
      if (chart.crosshairBehavior.enable &&
          cartesianSeriesRenderer != null &&
          SeriesHelper.getSeriesRendererDetails(cartesianSeriesRenderer).series is! ErrorBarSeries &&
          chart.crosshairBehavior.activationMode == ActivationMode.singleTap) {
        _stateProperties.crosshairBehaviorRenderer.onEnter(position.dx, position.dy);
      }
    }
  }
  ...
  void _performMouseExit(PointerEvent event) {
    if (_stateProperties.chartState.mounted) {
      TooltipHelper.getRenderingDetails(_stateProperties.renderingDetails.tooltipBehaviorRenderer).isHovering = false;
      final Offset position = renderBox.globalToLocal(event.position);
      if (chart.tooltipBehavior.enable || shouldShowAxisTooltip(_stateProperties)) {
        _stateProperties.renderingDetails.tooltipBehaviorRenderer.onExit(position.dx, position.dy);
      }
      if (chart.crosshairBehavior.enable) {
        _stateProperties.crosshairBehaviorRenderer.onExit(position.dx, position.dy);
      }
      if (chart.trackballBehavior.enable) {
        _stateProperties.trackballBehaviorRenderer.onExit(position.dx, position.dy);
      }
    }
  }
  ...
  void _bindInteractionWidgets(BoxConstraints constraints, BuildContext context) {
    ...
    final ZoomRectPainter zoomRectPainter = ZoomRectPainter(stateProperties: _stateProperties);
    final ZoomingBehaviorDetails zoomingBehaviorDetails = ZoomPanBehaviorHelper.getRenderingDetails(_stateProperties.zoomPanBehaviorRenderer);
    zoomingBehaviorDetails.painter = zoomRectPainter;
    CrosshairHelper.setStateProperties(chart.crosshairBehavior, _stateProperties);
    TooltipHelper.setStateProperties(chart.tooltipBehavior, _stateProperties);
    TrackballHelper.setStateProperties(chart.trackballBehavior, _stateProperties);
    ZoomPanBehaviorHelper.setStateProperties(chart.zoomPanBehavior, _stateProperties);
    // ignore: unnecessary_null_comparison
    if (chart.trackballBehavior != null && chart.trackballBehavior.enable) {
      if (chart.trackballBehavior.builder != null) {
        trackballRenderingDetails.trackballTemplate = TrackballTemplate(
            key: GlobalKey<State<TrackballTemplate>>(), trackballBehavior: chart.trackballBehavior, stateProperties: _stateProperties);
        userInteractionWidgets.add(trackballRenderingDetails.trackballTemplate!);
      } else {
        trackballPainter = TrackballPainter(stateProperties: _stateProperties, valueNotifier: _stateProperties.repaintNotifiers['trackball']!);
    ...
    if (chart.crosshairBehavior != null && chart.crosshairBehavior.enable) {
      crosshairPainter = CrosshairPainter(stateProperties: _stateProperties, valueNotifier: _stateProperties.repaintNotifiers['crosshair']!);
      CrosshairHelper.getRenderingDetails(_stateProperties.crosshairBehaviorRenderer).crosshairPainter = crosshairPainter;
      userInteractionWidgets.add(Container(
          height: constraints.maxHeight,
          width: constraints.maxWidth,
          decoration: const BoxDecoration(color: Colors.transparent),
    ...
    final TooltipRenderingDetails tooltipRenderingDetails = TooltipHelper.getRenderingDetails(_renderingDetails.tooltipBehaviorRenderer);
    if (chart.tooltipBehavior.enable || shouldShowAxisTooltip(_stateProperties)) {
      tooltipRenderingDetails.prevTooltipValue = tooltipRenderingDetails.currentTooltipValue = null;
      tooltipRenderingDetails.chartTooltip = SfTooltip(
          ...
          labelColor: tooltip.textStyle.color ?? _renderingDetails.chartTheme.tooltipLabelColor,
          header: tooltip.header,
          format: tooltip.format,
          shadowColor: tooltip.shadowColor,
          onTooltipRender: chart.onTooltipRender != null ? tooltipRenderingDetails.tooltipRenderingEvent : null);
      _renderingDetails.chartWidgets!.add(tooltipRenderingDetails.chartTooltip!);
    }
    final Widget uiWidget = IgnorePointer(ignoring: chart.annotations != null, child: Stack(children: userInteractionWidgets));
  ...
  void _triggerAxisLabelEvent(Offset position) {
    for (int i = 0; i < _stateProperties.chartAxis.axisRenderersCollection.length; i++) {
      final ChartAxisRendererDetails axisDetails = AxisHelper.getAxisRendererDetails(_stateProperties.chartAxis.axisRenderersCollection[i]);
      final List<AxisLabel> labels = axisDetails.visibleLabels;
      for (int k = 0; k < labels.length; k++) {
        if (axisDetails.axis.isVisible && AxisHelper.getLabelRegion(labels[k]) != null && AxisHelper.getLabelRegion(labels[k])!.contains(position)) {
  ...
  CustomPainter _getSeriesPainter(int value, AnimationController controller, CartesianSeriesRenderer seriesRenderer) {
    CustomPainter? customPainter;
    final PainterKey painterKey = PainterKey(index: value, name: 'series $value', isRenderCompleted: false);
    ...
    switch (seriesRendererDetails.seriesType) {
      case 'line':
        customPainter = LineChartPainter(
            ...
            isRepaint: _stateProperties.zoomedState != null
                ...
                : (_stateProperties.legendToggling || seriesRendererDetails.needsRepaint == true),
      ...
      case 'column':
        customPainter = ColumnChartPainter(
            ...
            isRepaint: !(_stateProperties.zoomedState != null) || _stateProperties.zoomedAxisRendererStates.isNotEmpty,
            painterKey: painterKey,
            animationController: controller,
            notifier: seriesRendererDetails.repaintNotifier);
        break;
      ...
      case 'bar':
        customPainter = BarChartPainter(
            ...
            isRepaint: ((_stateProperties.zoomedState != null) == false) || _stateProperties.zoomedAxisRendererStates.isNotEmpty,
            painterKey: painterKey,
            animationController: controller,
            notifier: seriesRendererDetails.repaintNotifier);
        break;
