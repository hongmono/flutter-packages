...
class ZoomPanBehavior {
  ...
  void zoomIn() {
    ...
    final ZoomPanBehaviorRenderer zoomPanBehaviorRenderer = _stateProperties.zoomPanBehaviorRenderer;
    ...
    final double? zoomFactor = zoomPanBehaviorRenderer._zoomingBehaviorDetails.zoomFactor;
    ...
    for (int index = 0; index < _stateProperties.chartAxis.axisRenderersCollection.length; index++) {
      axisDetails = AxisHelper.getAxisRendererDetails(_stateProperties.chartAxis.axisRenderersCollection[index]);
      if ((axisDetails.orientation == AxisOrientation.vertical && chart.zoomPanBehavior.zoomMode != ZoomMode.x) ||
          (axisDetails.orientation == AxisOrientation.horizontal && chart.zoomPanBehavior.zoomMode != ZoomMode.y)) {
        if (axisDetails.zoomFactor <= 1.0 && axisDetails.zoomFactor > 0.0) {
          if (axisDetails.zoomFactor - 0.1 < 0) {
            needZoom = false;
            break;
          } else {
            zoomPanBehaviorRenderer._zoomingBehaviorDetails.setZoomFactorAndZoomPosition(_stateProperties.chartState, axisDetails, zoomFactor);
            needZoom = true;
          }
        }
  ...
  void zoomOut() {
    ...
    for (int index = 0; index < _stateProperties.chartAxis.axisRenderersCollection.length; index++) {
      ...
      if ((axisDetails.orientation == AxisOrientation.vertical && chart.zoomPanBehavior.zoomMode != ZoomMode.x) ||
        ...
        if (axisDetails.zoomFactor < 1.0 && axisDetails.zoomFactor > 0.0) {
          zoomPanBehaviorRenderer._zoomingBehaviorDetails.setZoomFactorAndZoomPosition(_stateProperties.chartState, axisDetails, zoomFactor);
          axisDetails.zoomFactor = axisDetails.zoomFactor > 1.0 ? 1.0 : (axisDetails.zoomFactor < 0.0 ? 0.0 : axisDetails.zoomFactor);
        }
        if (chart.onZooming != null) {
          bindZoomEvent(chart, axisDetails, chart.onZooming!);
        }
  ...
  void zoomByFactor(double zoomFactor) {
    ...
    for (int index = 0; index < _stateProperties.chartAxis.axisRenderersCollection.length; index++) {
      ...
      _stateProperties.zoomPanBehaviorRenderer._zoomingBehaviorDetails.createZoomState();
    }
  }
  ...
  void zoomByRect(Rect rect) {
    _stateProperties.canSetRangeController = true;
    _stateProperties.zoomPanBehaviorRenderer._zoomingBehaviorDetails.doSelectionZooming(rect);
  }
  ...
  void zoomToSingleAxis(ChartAxis axis, double zoomPosition, double zoomFactor) {
    _stateProperties.canSetRangeController = true;
    final ChartAxisRenderer? axisRenderer = findExistingAxisRenderer(axis, _stateProperties.chartAxis.axisRenderersCollection);
  ...
  void panToDirection(String direction) {
    ...
    for (int axisIndex = 0; axisIndex < _stateProperties.chartAxis.axisRenderersCollection.length; axisIndex++) {
      axisDetails = AxisHelper.getAxisRendererDetails(_stateProperties.chartAxis.axisRenderersCollection[axisIndex]);
      if (axisDetails.orientation == AxisOrientation.horizontal) {
        if (direction == 'left') {
          axisDetails.zoomPosition =
              (axisDetails.zoomPosition > 0 && axisDetails.zoomPosition <= 0.9) ? axisDetails.zoomPosition - 0.1 : axisDetails.zoomPosition;
          axisDetails.zoomPosition = axisDetails.zoomPosition < 0.0 ? 0.0 : axisDetails.zoomPosition;
        }
        if (direction == 'right') {
          axisDetails.zoomPosition =
              (axisDetails.zoomPosition >= 0 && axisDetails.zoomPosition < 1) ? axisDetails.zoomPosition + 0.1 : axisDetails.zoomPosition;
          axisDetails.zoomPosition = axisDetails.zoomPosition > 1.0 ? 1.0 : axisDetails.zoomPosition;
        }
      } else {
        if (direction == 'bottom') {
          axisDetails.zoomPosition =
...
class ZoomPanBehaviorRenderer with ZoomBehavior {
  ...
  @override
  void onPan(double xPos, double yPos) => _zoomingBehaviorDetails.doPan(xPos, yPos);

  /// Performs the double-tap action.
  @override
  void onDoubleTap(double xPos, double yPos, double? zoomFactor) => _zoomingBehaviorDetails.doubleTapZooming(xPos, yPos, zoomFactor);

  /// Draws selection zoomRect.
  @override
  void onPaint(Canvas canvas) => _zoomingBehaviorDetails.painter.drawRect(canvas);

  /// Performs selection zooming.
  @override
  void onDrawSelectionZoomRect(double currentX, double currentY, double startX, double startY) =>
      _zoomingBehaviorDetails.drawSelectionZoomRect(currentX, currentY, startX, startY);
...
class ZoomingBehaviorDetails {
  ...
  void doubleTapZooming(double xPos, double yPos, double? zoomFactor) {
    ...
    for (int axisIndex = 0; axisIndex < stateProperties.chartAxis.axisRenderersCollection.length; axisIndex++) {
      axisDetails = AxisHelper.getAxisRendererDetails(stateProperties.chartAxis.axisRenderersCollection[axisIndex]);
      ...
      if ((axisDetails.orientation == AxisOrientation.vertical && _zoomPanBehavior.zoomMode != ZoomMode.x) ||
          (axisDetails.orientation == AxisOrientation.horizontal && _zoomPanBehavior.zoomMode != ZoomMode.y)) {
        cumulative = math.max(math.max(1 / _minMax(axisDetails.zoomFactor, 0, 1), 1) + (0.25), 1);
        if (cumulative >= 1) {
          ...
          origin = origin > 1
              ...
              : origin < 0
                  ? 0
                  : origin;
          zoomFactor = cumulative == 1 ? 1 : _minMax(1 / cumulative, 0, 1).toDouble();
          _zoomPosition = (cumulative == 1) ? 0 : axisDetails.zoomPosition + ((axisDetails.zoomFactor - zoomFactor) * origin);
          if (axisDetails.zoomPosition != _zoomPosition || axisDetails.zoomFactor != zoomFactor) {
            zoomFactor = (_zoomPosition! + zoomFactor) > 1 ? (1 - _zoomPosition!) : zoomFactor;
          }
  ...
  void doPan(double xPos, double yPos) {
    ...
    for (int axisIndex = 0; axisIndex < stateProperties.chartAxis.axisRenderersCollection.length; axisIndex++) {
      ...
      if ((axisDetails.orientation == AxisOrientation.vertical && _zoomPanBehavior.zoomMode != ZoomMode.x) ||
        ...
        if (axisDetails.orientation == AxisOrientation.horizontal) {
          value = (previousMovedPosition!.dx - xPos) / stateProperties.chartAxis.axisClipRect.width / currentScale;
          currentZoomPosition = _minMax(
                  axisDetails.axis.isInversed ? axisDetails.zoomPosition - value : axisDetails.zoomPosition + value, 0, 1 - axisDetails.zoomFactor)
              .toDouble();
          ...
          value = (previousMovedPosition!.dy - yPos) / stateProperties.chartAxis.axisClipRect.height / currentScale;
          currentZoomPosition = _minMax(
                  axisDetails.axis.isInversed ? axisDetails.zoomPosition + value : axisDetails.zoomPosition - value, 0, 1 - axisDetails.zoomFactor)
              .toDouble();
          axisDetails.zoomPosition = currentZoomPosition;
  ...
  void drawSelectionZoomRect(double currentX, double currentY, double startX, double startY) {
    stateProperties.canSetRangeController = true;
    final Rect clipRect = stateProperties.chartAxis.axisClipRect;
    final Offset startPosition = Offset((startX < clipRect.left) ? clipRect.left : startX, (startY < clipRect.top) ? clipRect.top : startY);
    final Offset currentMousePosition = Offset((currentX > clipRect.right) ? clipRect.right : ((currentX < clipRect.left) ? clipRect.left : currentX),
        (currentY > clipRect.bottom) ? clipRect.bottom : ((currentY < clipRect.top) ? clipRect.top : currentY));
    rectPath = Path();
  ...
  void doSelectionZooming(Rect zoomRect, {bool? isArea}) {
    stateProperties.canSetRangeController = true;
      ...
      for (int axisIndex = 0; axisIndex < stateProperties.chartAxis.axisRenderersCollection.length; axisIndex++) {
        ...
        if (axisDetails.orientation == AxisOrientation.horizontal) {
          if (_zoomPanBehavior.zoomMode != ZoomMode.y) {
            axisDetails.zoomPosition += ((zoomRect.left - rect.left) / (rect.width)).abs() * axisDetails.zoomFactor;
            axisDetails.zoomFactor *= zoomRect.width / rect.width;
            axisDetails.zoomFactor =
                axisDetails.zoomFactor >= _zoomPanBehavior.maximumZoomLevel ? axisDetails.zoomFactor : _zoomPanBehavior.maximumZoomLevel;
          }
        } else {
          if (_zoomPanBehavior.zoomMode != ZoomMode.x) {
            axisDetails.zoomPosition += (1 - ((zoomRect.height + (zoomRect.top - rect.top)) / (rect.height)).abs()) * axisDetails.zoomFactor;
  ...
  void performPinchZooming(List<PointerEvent> touchStartList, List<PointerEvent> touchMoveList) {
    stateProperties.canSetRangeController = true;
    num touch0StartX, touch0StartY, touch1StartX, touch1StartY, touch0EndX, touch0EndY, touch1EndX, touch1EndY;
    if (!(zoomingRect.width > 0 && zoomingRect.height > 0)) {
      ...
      Rect pinchRect;
      scaleX = (touch0EndX - touch1EndX).abs() / (touch0StartX - touch1StartX).abs();
      scaleY = (touch0EndY - touch1EndY).abs() / (touch0StartY - touch1StartY).abs();
      clipX = ((clipRect.left - touch0EndX) / scaleX) + math.min(touch0StartX, touch1StartX);
      clipY = ((clipRect.top - touch0EndY) / scaleY) + math.min(touch0StartY, touch1StartY);
      pinchRect = Rect.fromLTWH(clipX, clipY, clipRect.width / scaleX, clipRect.height / scaleY);
  ...
  void createZoomState() {
    ...
    stateProperties.zoomedAxisRendererStates.addAll(stateProperties.chartAxis.axisRenderersCollection);
  ...
  void _calculatePinchZoomFactor(SfCartesianChart chart, Rect pinchRect) {
    ...
    for (int axisIndex = 0; axisIndex < stateProperties.chartAxis.axisRenderersCollection.length; axisIndex++) {
      ...
      axisDetails.previousZoomFactor = axisDetails.zoomFactor;
      axisDetails.previousZoomPosition = axisDetails.zoomPosition;
      if ((axisDetails.orientation == AxisOrientation.horizontal && mode != ZoomMode.y) ||
          (axisDetails.orientation == AxisOrientation.vertical && mode != ZoomMode.x)) {
        if (axisDetails.orientation == AxisOrientation.horizontal) {
          ...
          rangeMin = (value * -1 + clipRect.height) / axisTrans + zoomAxes[axisIndex].min!;
          value = pinchRect.top + pinchRect.height - clipRect.top;
          rangeMax = (value * -1 + clipRect.height) / axisTrans + zoomAxes[axisIndex].min!;
        }
        selectionMin = math.min(rangeMin, rangeMax);
        selectionMax = math.max(rangeMin, rangeMax);
        currentPosition = (selectionMin - zoomAxes[axisIndex].actualMin!) / zoomAxes[axisIndex].actualDelta!;
        currentFactor = (selectionMax - selectionMin) / zoomAxes[axisIndex].actualDelta!;
        currentZoomPosition = currentPosition < 0 ? 0 : currentPosition;
        currentZoomFactor = currentFactor > 1 ? 1 : currentFactor;
        maxZoomFactor = _zoomPanBehavior.maximumZoomLevel;
        if (axisDetails.axis.autoScrollingDelta != null && axisDetails.scrollingDelta != null) {
          // To find zoom factor for corresponding auto scroll delta.
          minZoomFactor = axisDetails.scrollingDelta! / zoomAxes[axisIndex].actualDelta!;
        }
        ...
        stateProperties.zoomPanBehaviorRenderer.onPinch(axisDetails, currentZoomPosition, currentZoomFactor);
      }
      if (chart.onZooming != null) {
        bindZoomEvent(chart, axisDetails, chart.onZooming!);
      }
  ...
  num _minMax(num value, num min, num max) => value > max ? max : (value < min ? min : value);
  ...
  void _calculateZoomAxesRange(SfCartesianChart chart) {
    ...
    for (int index = 0; index < stateProperties.chartAxis.axisRenderersCollection.length; index++) {
      axisDetails = AxisHelper.getAxisRendererDetails(stateProperties.chartAxis.axisRenderersCollection[index]);
      range = ZoomAxisRange();
      if (axisDetails.visibleRange != null) {
        axisRange = axisDetails.visibleRange!;
      }
      if (stateProperties.zoomAxes.isNotEmpty && index <= stateProperties.zoomAxes.length - 1) {
        if (!delayRedraw) {
  ...
  void performMouseWheelZooming(PointerScrollEvent event, double mouseX, double mouseY) {
    stateProperties.canSetRangeController = true;
    ...
    for (int axisIndex = 0; axisIndex < stateProperties.chartAxis.axisRenderersCollection.length; axisIndex++) {
      ...
      if ((axisDetails.orientation == AxisOrientation.vertical && _zoomPanBehavior.zoomMode != ZoomMode.x) ||
          (axisDetails.orientation == AxisOrientation.horizontal && _zoomPanBehavior.zoomMode != ZoomMode.y)) {
        cumulative = math.max(math.max(1 / _minMax(axisDetails.zoomFactor, 0, 1), 1) + (0.25 * direction), 1);
        if (cumulative >= 1) {
          origin = axisDetails.orientation == AxisOrientation.horizontal
          ...
          origin = origin > 1
              ...
              : origin < 0
                  ...
                  : origin;
          zoomFactor = ((cumulative == 1) ? 1 : _minMax(1 / cumulative, 0, 1)).toDouble();
          zoomPosition = (cumulative == 1) ? 0 : axisDetails.zoomPosition + ((axisDetails.zoomFactor - zoomFactor) * origin);
          if (axisDetails.zoomPosition != zoomPosition || axisDetails.zoomFactor != zoomFactor) {
          ...
          if (axisDetails.zoomFactor.toInt() == 1 && axisDetails.zoomPosition.toInt() == 0 && _chart.onZoomReset != null) {
  ...
  void setZoomFactorAndZoomPosition(SfCartesianChartState chartState, ChartAxisRendererDetails axisDetails, double? zoomFactor) {
    ...
    final num direction = isZoomIn
        ...
        : isZoomOut
            ? -1
            : 1;
    final num cumulative = math.max(math.max(1 / _minMax(axisDetails.zoomFactor, 0, 1), 1) + (0.1 * direction), 1);
    if (cumulative >= 1) {
      num origin = axisDetails.orientation == AxisOrientation.horizontal
          ? (axisClipRect.left + axisClipRect.width / 2) / axisClipRect.width
          : 1 - ((axisClipRect.top + axisClipRect.height / 2) / axisClipRect.height);
      origin = origin > 1
          ? 1
          : origin < 0
              ? 0
...
class ZoomPanBehaviorHelper {
  ...
  static ZoomingBehaviorDetails getRenderingDetails(ZoomPanBehaviorRenderer renderer) {
    return renderer._zoomingBehaviorDetails;
  }

  /// Method to set the cartesian state properties.
  static void setStateProperties(ZoomPanBehavior zoomPanBehavior, CartesianStateProperties stateProperties) {
